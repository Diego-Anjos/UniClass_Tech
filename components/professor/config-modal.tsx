      ...
      ? createPortal(
          ...
          >
            <div
              ...
              className="w-full max-w-2xl lg:max-w-3xl bg-[#111827] border border-gray-800 rounded-xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]"
              onClick={(e) => e.stopPropagation()}
            >
            <div className="flex items-start justify-between gap-4 p-6 pb-4 border-b border-gray-800 shrink-0">
              <div>
                <h2
                  id="config-titulo"
              ...
              <button
                ...
                disabled={salvando}
                aria-label="Fechar"
                className="text-zinc-500 hover:text-white transition-colors cursor-pointer p-1 rounded-lg hover:bg-zinc-800 disabled:opacity-50 shrink-0"
              >
                <X className="w-5 h-5" />
              </button>
            </div>

            <div className="flex items-center gap-2 border-b border-gray-800 px-6 pt-3 pb-2 overflow-x-auto no-scrollbar whitespace-nowrap w-full shrink-0">
              {ABAS.map(({ id, label, icon: Icon }) => (
                <button
                  key={id}
                  type="button"
                  onClick={() => setAbaAtiva(id)}
                  className={`flex shrink-0 items-center gap-1.5 px-3 py-2.5 text-xs font-medium whitespace-nowrap rounded-t-lg border-b-2 transition-colors cursor-pointer ${
                    abaAtiva === id
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 p-6 border-t border-gray-800 shrink-0 bg-[#111827]">
              <button
                type="button"
